...
//ros communications:
  ...
  int main(int argc, char **argv) {
      ...
      bool get_red_path, get_blue_path, get_green_path;
      ...
      if (!(get_red_path && get_blue_path && get_green_path)){
          return 0;}
          else{
            ROS_INFO_STREAM(red_box_path << " has been extracted");
            ROS_INFO_STREAM(blue_box_path << " has been extracted");
            ROS_INFO_STREAM(green_box_path << " has been extracted");
}


      std::ifstream red_inXml, blue_inXml, green_inXml;
      std::stringstream red_strStream, blue_strStream, green_strStream;
      std::string red_xmlStr, blue_xmlStr, green_xmlStr;
      // red box
      ...
      // green box
      green_inXml.open(green_box_path.c_str());
      ...
      while (ros::ok()){
          ...
          int8_t color_index;  // 0 is red, 1 is blue, 2 is green

          spawn_model_req.initial_pose.position.y = (float)rand()/(float)(RAND_MAX) * 0.4;  // random between -0.4 to 0.4
          ROS_INFO_STREAM("y position of new blocks randomly: "
          << spawn_model_req.initial_pose.position.y);

        int random_value = rand() % 3; // Generate a random value between 0 and 2

        if (random_value == 0) {
            // Choose red block
            color_index = 0;
            model_name = "red_box_" + index;
            spawn_model_req.model_name = model_name;
            spawn_model_req.robot_namespace = "red_box_" + index;
            spawn_model_req.model_xml = red_xmlStr;
        }
        else if (random_value == 1) {
            // Choose blue block
            color_index = 1;
            model_name = "blue_box_" + index;
            spawn_model_req.model_name = model_name;
            spawn_model_req.robot_namespace = "blue_box_" + index;
            spawn_model_req.model_xml = blue_xmlStr;
        }
        else {
            // Choose green block
            color_index = 2;
            model_name = "green_box_" + index;
            spawn_model_req.model_name = model_name;
            spawn_model_req.robot_namespace = "green_box_" + index;
          ...
          // publish current blocks status, all blocks will be published
          // no matter if it's successfully spawned, or successfully initialized in speed
          current_blocks_msg.data.push_back(color_index);
          ...
          ros::spinOnce();
          ros::Duration(20.0).sleep();  // frequency control, spawn one box in each loop
          // delay time decides density of the box
